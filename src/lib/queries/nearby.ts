// Nearby station query (BUILD-CONTRACT §6). DUAL-SOURCE: with a DB we run the index-assisted
// PostGIS path (ST_DWithin to bound the radius, ordered by the KNN <-> operator, ST_Distance for the
// true-metre distance; spec §6.14 query (a)); without one we compute distance with geo.haversineKm
// over the seed dataset. Both paths return §5 StationWithDistance rows sorted nearest-first.
...
function mapRow(r: RawNearbyRow): StationWithDistance {
  return {
    id: r.id,
    slug: r.slug,
    name: r.name,
...
function nearbyFromSeed(
  ...
  return allStations()
    ...
    .map((s) => ({ ...s, distanceKm: haversineKm(c, { lat: s.lat, lng: s.lng }) }))
...
export async function nearbyStations(
  ...
  const db = getDb();
  if (!db) return nearbyFromSeed(c, radiusKm, f, limit);
  ...
  const rows = await db<RawNearbyRow[]>`
    SELECT
      s.public_id                     AS id,
      s.slug                          AS slug,
      s.name                          AS name,
      ...
      END                             AS "lastVerified",
      round((ST_Distance(s.location, me.g) / 1000.0)::numeric, 3)::float8 AS "distanceKm"
    FROM app.stations s
    CROSS JOIN (SELECT ST_SetSRID(ST_MakePoint(${c.lng}, ${c.lat}), 4326)::geography AS g) me
    ...
    WHERE s.status <> 'duplicate'::app.station_status
      ...
      AND ST_DWithin(s.location, me.g, ${radiusM})
      ${grades
